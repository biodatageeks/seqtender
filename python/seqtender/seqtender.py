...
class SeqTenderCommandBuilder:
    ...
    :param command: command which will be run - generated by CommandBuilder object (it maps input parameters to the command)
    :type command: String

    """

    def __init__(self, session: SparkSession, reads_path, index_path, tool, interleaved, read_group_id="",
                 read_group=""):
...
class SeqTenderAnnotation:
    ...
    def pipe_variants(self, path, command):
        return self.session._jvm.org.biodatageeks.annotation.SeqTenderVCF.pipeVCF(path, command, self.session._jsparkSession)
